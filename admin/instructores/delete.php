<?php
  ...
  $res = mysqli_query($conexion, "SELECT * FROM `instructores` WHERE `idInstructor`='".$idInstructor."'"); // Consulta para saber si ya existe ese valor
  ...
  if(!$res->num_rows > 0) { // Si la consulta dió algún registro significa que ya existe y entra en el if
    ...
    $response->mensaje = 'No existe ese instructor'; // Respuesta que se le dará al frontend
    echo json_encode($response); // Respuesta de la API
      exit();
  }else{
  ...
  if ($delete){ // Si funcionó la consulta entra en el if
    ...
    $response->mensaje = 'Instructor eliminado'; // Respuesta que se le dará al frontend
  }else{
