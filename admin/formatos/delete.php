<?php
  ...
  if (!isset($_GET['idFormato'])) {
    ...
    $idFormato = mysqli_real_escape_string($conexion,$_GET['idFormato']);
  }

  $res = mysqli_query($conexion, "SELECT * FROM `formatos` WHERE `idFormato`='".$idFormato."'"); // Consulta para saber si ya existe ese valor
...
if(!$res->num_rows > 0) { // Si la consulta dió algún registro significa que ya existe y entra en el if
  ...
  $delete = mysqli_query($conexion,"DELETE FROM `formatos` WHERE `formatos`.`idFormato`= '".$idFormato."'"); // Se obtiene de la peticion GET en la URL
