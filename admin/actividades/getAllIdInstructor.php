...
if ($_SERVER['REQUEST_METHOD'] != 'GET') {
  ...
  if (!isset($_GET['idTipoActividad']) && !isset($_GET['idInstructor'])) {
      $response->resultado = false;
  ...
  $registros=mysqli_query($conexion,"SELECT * FROM `actividades` inner join instructores ins on ins.idInstructor = actividades.idInstructor WHERE `idTipoActividad` = '".$idTipoActividad."' AND actividades.idInstructor = '".$idInstructor."'");
