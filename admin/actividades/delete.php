<?php
  ...
  $res = mysqli_query($conexion, "SELECT * FROM `carreras` WHERE `idCarrera`='".$idCarrera."'"); // Consulta para saber si ya existe ese valor
...
if(!$res->num_rows > 0) { // Si la consulta dió algún registro significa que ya existe y entra en el if
  ...
  $response->mensaje = 'No existe esa carrera'; // Respuesta que se le dará al frontend
  echo json_encode($response); // Respuesta de la API
    exit();
}else{

  // Consulta SQL que se debe aplicar para eliminar un registro
  $delete = mysqli_query($conexion,"DELETE FROM `actividades` WHERE `actividades`.`idActividad`= '".$idActividad."'"); // Se obtiene de la peticion GET en la URL

  if ($delete){ // Si funcionó la consulta entra en el if
    $response->resultado = true; // Mensaje de éxito porque ya se elimino
    $response->mensaje = 'Actividad eliminada'; // Respuesta que se le dará al frontend
  }else{
